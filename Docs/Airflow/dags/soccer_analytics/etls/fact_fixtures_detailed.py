...
query_round = f"""
    ...
    SELECT 
        ...
        ,CAST(fixture_status_elapsed AS INT) fixture_status_elapsed
        ...
        ,COALESCE(CAST(score_halftime_home AS INT)) AS score_halftime_home
        ,COALESCE(CAST(score_fulltime_home AS INT)) AS score_fulltime_home
        ,COALESCE(CAST(score_extratime_home AS INT)) AS score_extratime_home
        ,COALESCE(CAST(score_penalty_home AS INT)) AS score_penalty_home
        ,teams_away_id
        ,COALESCE(CAST(goals_away AS INT)) AS goals_away
        ,COALESCE(CAST(score_halftime_away AS INT)) AS score_halftime_away
        ,COALESCE(CAST(score_fulltime_away AS INT)) AS score_fulltime_away
        ,COALESCE(CAST(score_extratime_away AS INT)) AS score_extratime_away
        ,COALESCE(CAST(score_penalty_away AS INT)) AS score_penalty_away
